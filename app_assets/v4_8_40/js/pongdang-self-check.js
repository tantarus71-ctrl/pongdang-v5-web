// 퐁당퐁당 v4.8.42 경량 실행본 브라우저 자가 점검 도구
// 한글 주석: 실제 브라우저에서 DOM, CSS, JS, 이미지 경로 상태를 빠르게 확인한다.

(function () {
  'use strict';

  const CHECK_ID = 'pongdang-v4.8.42-self-check';
  ...
  function row(ok, label, detail) {
    return `<div style="display:flex;gap:7px;align-items:flex-start;margin:3px 0;">
      <span aria-hidden="true">${ok ? '✅' : '⚠️'}</span>
      <span><strong>${label}</strong>${detail ? `<br><small style="opacity:.82">${detail}</small>` : ''}</span>
    </div>`;
  }
  ...
  async function runCheck() {
    ...
    for (const asset of requiredAssets) {
      // 한글 주석: GitHub Pages/Netlify 환경에서 실제 파일 응답 여부를 확인한다.
      assetResults.push({ asset, ...(await headOk(asset)) });
    }
    ...
    const allOk = domOk && assetOk && renderOk;

    panel.innerHTML = `
      <div style="display:flex;justify-content:space-between;gap:8px;align-items:center;margin-bottom:6px;">
        <strong>v4.8.42 점검</strong>
      ...
      ${row(assetOk, '파일 경로', assetOk ? '필수 CSS/JS/SVG 응답 확인' : assetResults.filter((x) => !x.ok).map((x) => `${x.asset} (${x.status})`).join(', '))}
      ${row(renderOk, '렌더링', `구간 ${rendered.zones} · 모드 ${rendered.modes} · 메뉴 ${rendered.menu} · 물고기 ${rendered.fish} · 카드 ${rendered.cards}`)}
      <div style="margin-top:7px;padding-top:7px;border-top:1px solid rgba(255,255,255,.14);color:${allOk ? '#b9ffdf' : '#ffe0a8'};">
        ${allOk ? '전체 1차 점검 통과' : '보정 필요 항목 있음'}
      </div>
    `;
    document.getElementById('pdSelfCheckClose')?.addEventListener('click', () => panel.remove());
    console.info('[Pongdang self-check]', { domResults, assetResults, rendered, allOk });
    return { domResults, assetResults, rendered, allOk };
  }

  window.PongdangSelfCheck = { run: runCheck };

  if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', () => setTimeout(runCheck, 450), { once: true });
  } else {
