...
(() => {
  ...
  const CFG = { clickableDepth:.78, repickDelay:[900,1700], observeMs:[5200,8400], cooldown:[4200,7600], turnCooldown:950, maxDt:33, deadZone:28, hysteresis:8, edgeMargin:74 };
  ...
  function guideFishBackInside(f,now){
    const r=safeRect(),margin=CFG.edgeMargin;
    const outX=f.x<r.minX-margin||f.x>r.maxX+margin;
    const outY=f.y<r.minY-margin*.72||f.y>r.maxY+margin*.72;
    if(!outX&&!outY)return;
    f.targetX=rand(r.w*.34,r.w*.66);
    f.targetY=rand(r.h*zone.behavior.y[0],r.h*zone.behavior.y[1]);
    f.targetDepth=f.activeFront?zone.observeDepth:rand(zone.depth[0],Math.min(zone.depth[1],.68));
    f.targetSpeed=Math.max(zone.behavior.speed[0],f.speed*.72);
    f.nextTargetAt=now+rand(1600,2800);
    if(f.activeFront)releaseActiveFish(now);
  }
  function clampFishSoft(f){
    const r=safeRect(),margin=CFG.edgeMargin;
    f.x=clamp(f.x,r.minX-margin,r.maxX+margin);
    f.y=clamp(f.y,r.minY-margin*.72,r.maxY+margin*.72);
  }
  ...
  function updateMotion(f,now,dt){if(now>f.nextTargetAt)pickNewTarget(f,now);guideFishBackInside(f,now);updateDesiredDirection(f);startTurnIfNeeded(f,now);updateTurn(f,now);const dx=f.targetX-f.x,dy=f.targetY-f.y,dist=Math.hypot(dx,dy)||1,nx=dx/dist,ny=dy/dist;f.speed=lerp(f.speed,f.targetSpeed,.032);f.orbitAngle+=(0.0046+f.speed*.0054)*(dt/16.67)*(f.id%2?1:-1);const turnBrake=f.isTurning?lerp(.40,.76,f.turnProgress):1;let ox=0,oy=0;if(f.pattern==='circle'){ox=Math.cos(f.orbitAngle+f.phase)*f.orbitRadius*.018;oy=Math.sin(f.orbitAngle+f.phase)*f.orbitRadius*.012}else if(f.pattern==='ellipse'){ox=Math.cos(f.orbitAngle+f.phase)*f.orbitRadius*.027;oy=Math.sin(f.orbitAngle+f.phase)*f.orbitRadius*.009}else if(f.pattern==='s'||f.pattern==='flow'){oy=Math.sin(f.orbitAngle+f.phase)*f.orbitRadius*.014;ox=Math.cos(f.orbitAngle*.7+f.phase)*f.orbitRadius*.009}else if(f.pattern==='deep'||f.pattern==='pause'){oy=Math.sin(f.orbitAngle*.55+f.phase)*f.orbitRadius*.006;ox=Math.cos(f.orbitAngle*.42+f.phase)*f.orbitRadius*.006}else{ox=Math.sin(f.orbitAngle+f.phase)*f.orbitRadius*.014;oy=Math.abs(Math.cos(f.orbitAngle+f.phase))*f.orbitRadius*.009}const flowPush=zone.flow*.065;f.x+=((nx*f.speed*1.78)+ox+flowPush*f.dir)*turnBrake*(dt/16.67);f.y+=((ny*f.speed*1.12)+oy+Math.sin(now*.0011+f.phase)*.035)*turnBrake*(dt/16.67);f.depth=lerp(f.depth,f.targetDepth,f.activeFront?.022:.012);clampFishSoft(f)}
  function getFishW(f){const r=safeRect(),base=r.w<480?112:r.w<900?138:160;return base*(.52+f.depth*.72)}
  function avoidCollision(){for(let loop=0;loop<6;loop++)for(let i=0;i<fishes.length;i++)for(let j=i+1;j<fishes.length;j++){const a=fishes[i],b=fishes[j],dx=b.x-a.x,dy=b.y-a.y,wSum=getFishW(a)+getFishW(b),minX=wSum*.30,minY=wSum*.088,sameDepth=Math.abs(a.depth-b.depth)<.17;if(Math.abs(dx)<minX&&Math.abs(dy)<minY&&sameDepth){const sx=dx>=0?1:-1,sy=dy>=0?1:-1,xPush=(minX-Math.abs(dx)+2)*.035,yPush=(minY-Math.abs(dy)+2)*.18;if(a.activeFront&&!b.activeFront){b.x+=sx*xPush*1.25;b.y+=sy*yPush*1.25;b.targetDepth=Math.max(zone.depth[0],b.targetDepth-.055);b.targetSpeed=Math.min(b.targetSpeed,zone.behavior.speed[1]*.72)}else if(b.activeFront&&!a.activeFront){a.x-=sx*xPush*1.25;a.y-=sy*yPush*1.25;a.targetDepth=Math.max(zone.depth[0],a.targetDepth-.055);a.targetSpeed=Math.min(a.targetSpeed,zone.behavior.speed[1]*.72)}else{a.x-=sx*xPush*.55;b.x+=sx*xPush*.55;a.y-=sy*yPush*.5;b.y+=sy*yPush*.5;a.targetDepth=clamp(a.targetDepth-.012,zone.depth[0],zone.depth[1]);b.targetDepth=clamp(b.targetDepth+.012,zone.depth[0],zone.depth[1])}clampFishSoft(a);clampFishSoft(b)}}}
  ...
  function releaseActiveFish(now){if(activeFishId==null)return;const f=fishes.find(x=>x.id===activeFishId);if(!f)return;const r=safeRect();f.activeFront=false;f.clickable=false;f.cooldownUntil=now+rand(CFG.cooldown[0],CFG.cooldown[1]);f.targetDepth=rand(zone.depth[0],Math.min(.62,zone.depth[1]));f.targetX=clamp(f.x+(f.dir*rand(90,170)),r.minX,r.maxX);f.targetY=rand(r.h*zone.behavior.y[0],r.h*zone.behavior.y[1]);lastActiveFishId=f.id;activeFishId=null;nextPickAt=now+rand(CFG.repickDelay[0],CFG.repickDelay[1]);f.el.root.classList.remove('clickable')}
  ...
  function enforceSingleActiveFish(now){const clickable=fishes.filter(f=>f.clickable);if(clickable.length>1){const keeper=clickable.find(f=>f.id===activeFishId)||clickable.sort((a,b)=>b.depth-a.depth)[0];clickable.forEach(f=>{if(f!==keeper){f.clickable=false;f.activeFront=false;f.cooldownUntil=now+rand(1200,2200);f.el.root.classList.remove('clickable')}});activeFishId=keeper?keeper.id:null}const fronts=fishes.filter(f=>f.activeFront);if(fronts.length>1){const keeper=fronts.find(f=>f.id===activeFishId)||fronts.sort((a,b)=>b.depth-a.depth)[0];fronts.forEach(f=>{if(f!==keeper){f.activeFront=false;f.targetDepth=rand(zone.depth[0],Math.min(.62,zone.depth[1]));f.cooldownUntil=now+rand(900,1800)}})}}
