(() => {
  ...
  function guessMeta(text){
    ...
    if(t.includes('피라미')||t.includes('piram')) return CARD_META.piramii;
    if(t.includes('쉬리')||t.includes('siri')) return CARD_META.siri;
    if(t.includes('각시')||t.includes('gaksi')) return CARD_META.gaksi;
    return CARD_META.beodeulchi;
  }
  ...
  function enhanceCards(){
    document.querySelectorAll('.dex-card').forEach(card=>{
      if(card.dataset.collectorReady==='true') return;
      const meta=guessMeta(card.textContent);
  ...
  function enhanceDetail(){
    ...
    if(detail && detail.dataset.collectorReady!=='true'){
      detail.dataset.collectorReady='true';detail.classList.add('collector-detail');
      const meta=guessMeta(detail.textContent);
    ...
    if(acquire && acquire.dataset.collectorReady!=='true'){
  ...
  window.PondangCollectorCardV30A1K={run,meta:CARD_META};
})();
